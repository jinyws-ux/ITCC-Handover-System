...
def serialize_timeline_event(event_type: str, event_time: datetime, task: Task, content: str, actor: str = "", log_type: str = "") -> dict:
    return {"eventType": event_type, "eventTime": event_time.isoformat(), "task": serialize_task_card(task), "content": content, "actor": actor, "logType": log_type, "externalLinks": [{"type": x.external_type, "id": x.external_id, "title": x.external_title, "status": x.external_status, "url": x.external_url} for x in task.external_links]}
...
def register_routes(app: Flask) -> None:
    ...
    @app.get("/api/auth/me")
    def me():
        user = current_user(); return jsonify({"user": serialize_user(user) if user else None})
    ...
    def create_task():
        ...
        db.session.commit(); return jsonify(serialize_task_detail(task)), 201
    ...
    def add_external_link(task_id: int):
        ...
        external_type = str(payload.get("externalType", "")).strip(); external_id = str(payload.get("externalId", "")).strip(); external_title = str(payload.get("externalTitle", "")).strip(); external_url = str(payload.get("externalUrl", "")).strip(); external_status = str(payload.get("externalStatus", "")).strip(); remark = str(payload.get("remark", "")).strip()
    ...
    def add_task_log(task_id: int):
        user = current_user(); task = Task.query.get_or_404(task_id); payload = request.get_json(silent=True) or {}; content = str(payload.get("content", "")).strip()
        if not content:
            return jsonify({"message": "Content is required."}), 400
        db.session.add(TaskLog(task=task, log_type=payload.get("logType", "Note"), content=content, created_by=user.id if user else None)); task.updated_by = user.id if user else task.updated_by; db.session.commit(); return jsonify(serialize_task_detail(task))

    @app.post("/api/tasks/<int:task_id>/status")
    def update_task_status(task_id: int):
        user = current_user(); task = Task.query.get_or_404(task_id); payload = request.get_json(silent=True) or {}; new_status = str(payload.get("status", "")).strip()
        if not new_status:
            return jsonify({"message": "Status is required."}), 400
        old_status = task.status; task.status = new_status; task.updated_by = user.id if user else task.updated_by
        if new_status == "Closed": task.closed_at = datetime.utcnow(); task.closed_by = user.id if user else task.closed_by
        db.session.add(TaskLog(task=task, log_type="Status", content=f"Status changed from {old_status} to {new_status}.", old_status=old_status, new_status=new_status, created_by=user.id if user else None)); db.session.commit(); return jsonify(serialize_task_detail(task))

    @app.post("/api/tasks/<int:task_id>/ack")
    def acknowledge_task(task_id: int):
        user = current_user(); task = Task.query.get_or_404(task_id); handover = Handover.query.filter_by(task_id=task.id).order_by(Handover.handed_over_at.desc()).first()
        if handover:
            handover.acknowledged_by = user.id if user else None; handover.acknowledged_at = datetime.utcnow(); handover.status = "Acknowledged"
        db.session.add(TaskLog(task=task, log_type="Ack", content="Handover acknowledged.", created_by=user.id if user else None)); db.session.commit(); return jsonify(serialize_task_detail(task))

    @app.post("/api/tasks/<int:task_id>/accept")
    def accept_task(task_id: int):
        user = current_user(); task = Task.query.get_or_404(task_id); handover = Handover.query.filter_by(task_id=task.id).order_by(Handover.handed_over_at.desc()).first()
        if handover:
            handover.accepted_by = user.id if user else None; handover.accepted_at = datetime.utcnow(); handover.status = "Accepted"
        task.status = "In Progress"; db.session.add(TaskLog(task=task, log_type="Accept", content="Handover accepted and task moved to In Progress.", new_status="In Progress", created_by=user.id if user else None)); db.session.commit(); return jsonify(serialize_task_detail(task))

    @app.get("/api/timeline")
    def timeline():
        tasks = apply_task_filters(Task.query).order_by(Task.updated_at.desc()).all()
        return jsonify({"items": [serialize_task_card(x) for x in tasks], "total": len(tasks), "stats": {"open": Task.query.filter(Task.status != "Closed").count(), "closed": Task.query.filter(Task.status == "Closed").count(), "monitoring": Task.query.filter(Task.is_monitoring.is_(True)).count(), "waitingNextShift": Task.query.filter(Task.status == "Waiting Next Shift").count()}})

    @app.get("/api/timeline-rich")
    def timeline_rich():
        tasks = apply_task_filters(Task.query).order_by(Task.updated_at.desc()).all()
        events = []
        for task in tasks:
            events.append(serialize_timeline_event("task_updated", task.updated_at, task, task.next_action or task.description or "Task updated", log_type="Task"))
            for log in task.logs:
                events.append(serialize_timeline_event("log", log.created_at, task, log.content, log_type=log.log_type))
            for handover in task.handovers:
                events.append(serialize_timeline_event("handover", handover.handed_over_at, task, handover.handover_note or "Handover recorded", log_type="Handover"))
            for link in task.external_links:
                events.append(serialize_timeline_event("external", link.created_at, task, f"{link.external_type}: {link.external_id or link.external_title or link.external_url}", log_type="External"))
        events.sort(key=lambda x: x["eventTime"], reverse=True)
        return jsonify({"items": [serialize_task_card(x) for x in tasks], "events": events[:250], "total": len(tasks)})
    ...
    def calendar():
        ...
        schedules = query.order_by(Schedule.work_date.asc(), Schedule.shift_id.asc()).all(); items = [serialize_schedule(x) for x in schedules]; by_date: dict[str, list[dict]] = {}
        for item in items: by_date.setdefault(item["date"], []).append(item)
    ...
    def calendar_rich():
        ...
        today = date.today()
        hypercare_start = max(start, today)
    ...
    def admin_create_schedule():
        ...
        payload = request.get_json(silent=True) or {}; work_date_raw = str(payload.get("workDate", "")).strip(); shift_id = payload.get("shiftId"); group_id = payload.get("groupId")
        if not work_date_raw or not shift_id or not group_id: return jsonify({"message": "Work date, shift, and group are required."}), 400
        try: work_date = datetime.strptime(work_date_raw, "%Y-%m-%d").date()
        except ValueError: return jsonify({"message": "Invalid work date format. Use YYYY-MM-DD."}), 400
        shift = Shift.query.get(shift_id)
        if not shift: return jsonify({"message": "Shift not found."}), 400
        schedule = Schedule(work_date=work_date, shift_id=shift_id, group_id=group_id, start_time=str(payload.get("startTime") or shift.default_start_time), end_time=str(payload.get("endTime") or shift.default_end_time), members_text=str(payload.get("members", "")).strip(), remark=str(payload.get("remark", "")).strip(), created_by=user.id if user else None)
    ...
    def admin_create_group():
        ...
        if not name: return jsonify({"message": "Group name is required."}), 400
        if Group.query.filter_by(name=name).first(): return jsonify({"message": "Group already exists."}), 400
        db.session.add(Group(name=name, description=str(payload.get("description", "")).strip())); db.session.commit(); return jsonify(meta_payload()), 201
    ...
    def admin_create_factory():
        ...
        if not name or not code: return jsonify({"message": "Factory name and code are required."}), 400
        if Factory.query.filter((Factory.name == name) | (Factory.code == code)).first(): return jsonify({"message": "Factory already exists."}), 400
        db.session.add(Factory(name=name, code=code)); db.session.commit(); return jsonify(meta_payload()), 201

    @app.post("/api/admin/systems")
    def admin_create_system():
        _, error = require_admin()
        if error: return error
        payload = request.get_json(silent=True) or {}; name = str(payload.get("name", "")).strip(); code = str(payload.get("code", "")).strip().upper(); factory_id = payload.get("factoryId") or None
        if not name or not code: return jsonify({"message": "System name and code are required."}), 400
        db.session.add(System(name=name, code=code, factory_id=factory_id)); db.session.commit(); return jsonify(meta_payload()), 201

    @app.post("/api/admin/users")
    def admin_create_user():
        _, error = require_admin()
        if error: return error
        payload = request.get_json(silent=True) or {}; username = str(payload.get("username", "")).strip(); password = str(payload.get("password", "123456")); display_name = str(payload.get("displayName", username)).strip()
        if not username or not display_name: return jsonify({"message": "Username and display name are required."}), 400
        if User.query.filter_by(username=username).first(): return jsonify({"message": "User already exists."}), 400
        db.session.add(User(username=username, password_hash=generate_password_hash(password), display_name=display_name, email=str(payload.get("email", "")).strip(), role=str(payload.get("role", "user")).strip() or "user", group_id=payload.get("groupId") or None)); db.session.commit(); return jsonify(meta_payload()), 201
